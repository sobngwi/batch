 ...
 *
 */
public class CustomerLookupItemProcessor implements
	...
	public Object process(Object curItem) throws Exception {
		if (curItem instanceof Customer) {
			...
			throw new InvalidItemException("An invalid item was received: "
					+ curItem);
		}
		return curItem;
	}
	...
	private void updateAccount(Transaction curItem) {
		Account account = accountDao.findAccountByNumber(curItem
				.getAccountNumber());
	...
	private void doCustomerUpdate(Customer curCustomer) {
		Customer storedCustomer = customerDao.findCustomerByTaxId(curCustomer
				.getTaxId());
		Account account = accountDao.findAccountByNumber(curCustomer
